...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO categories (name, slug, description, image, parent_id) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$name, $slug, $description, $image, $parent_id]);
    ...
    <div class="card" style="max-width: 800px; margin: 0 auto;">
        ...
        <form method="POST">
            <div class="form-group">
                ...
                <input type="text" name="name" class="form-input" placeholder="e.g., Electronics"
                    value="<?php echo htmlspecialchars($_POST['name'] ?? ''); ?>" required>
            </div>

            <div class="form-group">
                <label class="form-label">Slug *</label>
                <input type="text" name="slug" class="form-input" placeholder="e.g., electronics"
            ...
            <div class="form-group">
                ...
                <div class="image-upload-container" style="margin-top: 10px;">
                    ...
                    <button type="button" id="choose_image_btn" class="btn btn-secondary">Select Image</button>
                    <button type="button" class="btn btn-danger" id="remove_image_btn"
...
<script>
    $(document).ready(function () {
        ...
        const initialUrl = $('#category_image').val();
        if (initialUrl) {
            updateImagePreview(initialUrl);
        }
    });
</script>
