...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT COUNT(*) FROM footers WHERE slug = ?");
            $stmt->execute([$slug]);
            if ($stmt->fetchColumn() > 0) {
                $error = 'Slug already exists';
            } else {
                // Insert
                $stmt = $pdo->prepare("INSERT INTO footers (title, slug, status, content) VALUES (?, ?, 'draft', '[]')");
                $stmt->execute([$title, $slug]);

                $newId = $pdo->lastInsertId();
                header("Location: ../builder.php?type=footer&id=$newId"); // Redirect to builder
                exit;
            }
        } catch (Exception $e) {
            $error = $e->getMessage();
...
<div class="page-header">
    <h1 class="page-title">Create New Footer</h1>
</div>
    ...
    <div class="card" style="max-width: 600px; margin: 0 auto;">
        ...
        <form method="POST" action="">
            <div class="form-group">
                <label class="form-label" for="title">Footer Title</label>
                <input type="text" id="title" name="title" class="form-input" placeholder="e.g. Main Footer" required
                    autofocus>
            </div>
