...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT COUNT(*) FROM mobile_menus WHERE slug = ?");
            $stmt->execute([$slug]);
            if ($stmt->fetchColumn() > 0) {
                $error = 'Slug already exists';
            } else {
                // Insert
                $stmt = $pdo->prepare("INSERT INTO mobile_menus (title, slug, content) VALUES (?, ?, '[]')");
                $stmt->execute([$title, $slug]);

                $newId = $pdo->lastInsertId();
                $newId = $pdo->lastInsertId();
                header("Location: ../builder.php?type=mobile_menu&id=$newId"); // Redirect to builder
                exit;
                exit;
            }
        } catch (Exception $e) {
...
$pageTitle = 'Create Mobile Menu';
require_once __DIR__ . '/../includes/header.php';
?>

<div class="page-header">
    <h1 class="page-title">Create New Mobile Menu</h1>
</div>
    ...
    <div class="card" style="max-width: 600px; margin: 0 auto;">
        ...
        <form method="POST" action="">
            <div class="form-group">
                <label class="form-label" for="title">Mobile Menu Title</label>
