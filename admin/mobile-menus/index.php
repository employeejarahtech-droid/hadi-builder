...
try {
    ...
    // Get total count
    $count_stmt = $pdo->query("SELECT COUNT(*) FROM mobile_menus");
    $total_items = $count_stmt->fetchColumn();
    $total_pages = ceil($total_items / $items_per_page);
    
    // Fetch mobile menus with pagination
    $stmt = $pdo->prepare("SELECT * FROM mobile_menus ORDER BY created_at DESC LIMIT :limit OFFSET :offset");
    $stmt->bindValue(':limit', $items_per_page, PDO::PARAM_INT);
    $stmt->bindValue(':offset', $offset, PDO::PARAM_INT);
    $stmt->execute();
    $mobile_menus = $stmt->fetchAll();
} catch (Exception $e) {
    die("Error: " . $e->getMessage());
}

$currentPage = 'mobile_menus';
$pageTitle = 'Manage Mobile Menus';
require_once __DIR__ . '/../includes/header.php';
?>

<div class="page-header">
    <div style="display: flex; justify-content: space-between; align-items: center; padding-bottom: 1rem">
        <h1 class="page-title">Mobile Menus</h1>
        <a href="create.php" class="btn btn-primary">
            <i class="fa fa-plus"></i> Create New Mobile Menu
        </a>
    </div>
</div>

<div class="content-wrapper">
    <?php if (count($mobile_menus) > 0): ?>
        <table class="table">
            <thead>
                <tr>
                ...
                <?php foreach ($mobile_menus as $menu): ?>
                    <tr>
                        <td style="text-align: center;">
                            <input type="radio" name="default_mobile_menu" 
                                   value="<?php echo $menu['id']; ?>" 
                                   <?php echo (isset($menu['is_default']) && $menu['is_default']) ? 'checked' : ''; ?>
                                   onchange="setDefault(<?php echo $menu['id']; ?>)">
                        </td>
                        <td>
                            <input type="text" 
                                   ...
                                   onblur="updateTitle(this, <?php echo $menu['id']; ?>)"
                                   onkeypress="if(event.key === 'Enter') this.blur();">
                        </td>
                        <td>
                            <a href="<?php echo base_url . '/' . htmlspecialchars($menu['slug']); ?>" target="_blank"
                                style="color: var(--primary); text-decoration: none;">
                                /<?php echo htmlspecialchars($menu['slug']); ?> <i class="fa fa-external-link-alt"
                                    style="font-size: 0.75em;"></i>
                            </a>
                        </td>
                        <td>
                            <!-- Status column not used in mobile menus mostly, but keeping structure -->
                             <span class="badge badge-success">Published</span>
                        </td>
                        <td><?php echo date('M d, Y', strtotime($menu['updated_at'])); ?></td>
                        <td style="text-align: right;">
                            <a href="../builder.php?type=mobile_menu&id=<?php echo $menu['id']; ?>" class="btn btn-primary"
                                style="padding: 0.25rem 0.5rem; font-size: 0.75rem;">
                                <i class="fa fa-edit"></i> Edit
                            </a>
                            <a href="delete.php?id=<?php echo $menu['id']; ?>" class="btn btn-danger"
        ...
        <?php if ($total_pages > 1): ?>
            ...
            <div class="pagination"
                ...
                <div style="font-size: 0.875rem; color: #64748b;">
                    ...
                    <?php echo $total_items; ?> mobile menus
                </div>
    ...
    <?php else: ?>
        <div class="card" style="text-align: center; padding: 3rem;">
            <i class="fa fa-bars" style="font-size: 3rem; color: #cbd5e1; margin-bottom: 1rem;"></i>
            <h3>No mobile menus found</h3>
            <p style="color: var(--secondary); margin-bottom: 1.5rem;">Get started by creating your first mobile menu.</p>
            <a href="create.php" class="btn btn-primary">Create Mobile Menu</a>
        </div>
    <?php endif; ?>
</div>
