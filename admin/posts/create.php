    ...
    if (empty($title)) {
        ...
        try {
            ...
            if ($stmt->fetchColumn() > 0) {
                ...
                // Insert
                $stmt = $pdo->prepare("INSERT INTO posts (title, slug, status, content, meta_title, meta_description, meta_keywords, image, meta_image) VALUES (?, ?, 'draft', '[]', ?, ?, ?, ?, ?)");
                $stmt->execute([
                    $title,
                    $slug,
                ...
                header("Location: ../builder.php?type=post&id=$newId"); // Redirect to builder
                exit;
            }
        } catch (Exception $e) {
            $error = $e->getMessage();
    ...
    <form method="POST" action="" style="max-width: 800px; margin: 0 auto;">
        ...
        <div class="card" style="margin-bottom: 2rem;">
            ...
            <div class="form-group">
                ...
                <input type="text" id="title" name="title" class="form-input" placeholder="e.g. My First Post" required
                    autofocus>
            </div>

            <div class="form-group">
                <label class="form-label" for="slug">Slug (URL)</label>
                <input type="text" id="slug" name="slug" class="form-input" placeholder="e.g. my-first-post">
