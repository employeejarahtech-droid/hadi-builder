...
try {
    ...
    $sql = "SELECT * FROM projects $where_sql ORDER BY created_at DESC LIMIT :limit OFFSET :offset";
                ...
                <tbody>
                    <?php foreach ($projects as $project): ?>
                        <tr>
                            ...
                            <td>
                                <a href="<?php echo base_url . '/projects/' . htmlspecialchars($project['slug']); ?>"
                                    target="_blank" style="color: var(--primary); text-decoration: none;">
                                    /projects/
                                    <?php echo htmlspecialchars($project['slug']); ?> <i class="fa fa-external-link-alt"
                                        style="font-size: 0.75em;"></i>
                                </a>
                            </td>
                            <td>
                                <span
                                    class="badge <?php echo $project['status'] === 'published' ? 'badge-success' : 'badge-warning'; ?>">
                            ...
                            <td style="text-align: right;">
                                <div style="display: flex; gap: 0.5rem; justify-content: flex-end;">
                                    <button class="btn btn-sm toggle-status-btn" data-id="<?php echo $project['id']; ?>"
                                        data-status="<?php echo $project['status']; ?>"
                                        style="padding: 0.25rem 0.5rem; font-size: 0.75rem; white-space: nowrap; <?php echo $project['status'] === 'published' ? 'background: #f59e0b; border-color: #f59e0b;' : 'background: #10b981; border-color: #10b981;'; ?>"
                                        title="<?php echo $project['status'] === 'published' ? 'Unpublish' : 'Publish'; ?>">
                                        <i
                                            class="fa <?php echo $project['status'] === 'published' ? 'fa-eye-slash' : 'fa-eye'; ?>"></i>
                                        <?php echo $project['status'] === 'published' ? 'Unpublish' : 'Publish'; ?>
                                    </button>
                                    <a href="edit.php?id=<?php echo $project['id']; ?>" class="btn btn-secondary"
                                        style="padding: 0.25rem 0.5rem; font-size: 0.75rem;">
                                        <i class="fa fa-edit"></i>
                                    </a>
                                    <a href="delete.php?id=<?php echo $project['id']; ?>" class="btn btn-danger"
                                        style="padding: 0.25rem 0.5rem; font-size: 0.75rem;"
        ...
        <?php if ($total_pages > 1): ?>
            ...
            <div class="pagination"
                ...
                <div style="font-size: 0.875rem; color: #64748b;">
                    ...
                    <?php echo min($offset + $items_per_page, $total_projects); ?> of
                    <?php echo $total_projects; ?> projects
                </div>
    ...
    <?php else: ?>
        <div class="card" style="text-align: center; padding: 3rem;">
            ...
            <?php else: ?>
                ...
                <p style="color: var(--secondary); margin-bottom: 1.5rem;">Get started by creating your first project.</p>
                <a href="create.php" class="btn btn-primary">Create Project</a>
            <?php endif; ?>
        </div>
    <?php endif; ?>
