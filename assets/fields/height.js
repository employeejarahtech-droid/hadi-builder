console.log('🔧 HEIGHT.JS LOADED - Standalone Version - ' + new Date().toISOString());

class HEIGHT extends BaseControl {
    constructor(id, args) {
        super(id, args);
        this.type = 'height';
        this.label = args.label || 'Height';
        this.units = args.units || args.size_units || ['px', '%', 'rem', 'em', 'vh', 'vw']; // Default units for Height
        this.defaultValue = args.default_value || { size: '', unit: 'px' };
        ...
        return `
            <div class="elementor-control" data-control-id="${this.id}">
                ...
                <div class="elementor-control-input-wrapper">
                    <div class="height-control-wrapper" data-id="${this.id}" style="display: flex; gap: 5px; align-items: center;">
                        <input type="number" class="height-input elementor-control-input" id="${this.id}" 
    ...
    getValue() {
        ...
        const wrapper = input?.closest('.height-control-wrapper');

        if (!input || !wrapper) return this.value; // Return current stored value
    ...
    setValue(value) {
        super.setValue(value); // Emit change
    ...
    static init(args, selector) {
        const id = args[0];
        const instance = new HEIGHT(id, args[1]);
