...
<div id="category">
	...
	<div id="right_c"><select name="category" id="category" class="list_menu category">
				...
				<?php $sql_pp = "select DISTINCT category from gallery";
...
<div class="_add_new_photo_area">
	...
	<?php 	
			...
			if(!isset($category) || ($category=="")) {
				$sql1 = "select * from gallery where category='' order by id asc";
			} else {
				$sql1 = "select * from gallery where category  LIKE '%".$category."'";
			}
  		...
  		<div class="_detail_common">
			...
			<span>
				<div id="menutittle">
					...
					<div id="image_1"  dir="click here and change picture" class="no_image tooltip"><input id="<?php echo $result1["id"]; ?>" class="sub_category_edit" value="<?php echo $result1["sub_category"]; ?>" type="text"  onkeypress="if_enter_press_subcat('<?php echo $result1["id"]; ?>', this.value);" /></div>
					<div id="image_2"  dir="click here and change picture" class="no_image2 tooltip2"><input id="<?php echo $result1["id"]; ?>" class="sub_category_edit2" value="<?php echo $result1["more_sub_category"]; ?>" type="text"  onkeypress="if_enter_press_more_subcat('<?php echo $result1["id"]; ?>', this.value);" /></div>
			</div>
			</span> </div>
			</div>
		...
		(
			{
				...
				success: function (data, status)
				{
					...
					if(typeof(data.error) != 'undefined')
					{
						...
						$("#partner_image"+admno+"").replaceWith('<img id="partner_image'+admno+'" src="../../../../images/thmb_'+$.trim(data)+'" width="169px" height="119px" />');*/
...
$('.sub_category_edit2').keyup(function(e){
    if(e.keyCode == 13)
    {
       // $(this).trigger("enterKey");
	  // alert($(this).attr('id'));
...
$('.sub_category_edit').keyup(function(e){
    if(e.keyCode == 13)
    {
       // $(this).trigger("enterKey");
	  // alert($(this).attr('id'));
