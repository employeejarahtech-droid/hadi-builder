	...
	<div id="field_content">
		...
		<div id="field"><span class="_sponsor_input">
		 ...
		 <select name="link_types" id="link_types" class="list_menu">
				...
				<?php $sqls = "select * from link_type";
					$results = mysql_query($sqls);
					$selected = "";

					while ($rows = mysql_fetch_array($results)) {
				
						if($row['link_type_id'] == $rows['id']) {
							$selected  = "selected";
						}
					?>
...
function update(){   
	...
	if(valid) {
		...
		$.ajax({
				type: 'POST',
				url: 'settings/link/link_update_save_del.php',	
				data: { name: name, otherename: otherename, bi: bi, ind: ind, bn: bn, kr: kr, sp: sp, pr: pr, jp: jp, dt: dt, it: it, chi: chi, link_types: link_types,link_page: link_page, id: id, action: "update"  },
				success: function(data) {
						//alert(data);
						maskPage();
						if($.trim(data)=="1"){
