...
<div id="Addnew">
	...
	<div id="header_text">List of Link <div style="float:right; margin-right:200px;">
	...
	<div id="field_content">
		...
		<div id="field_shortname">Others(<?php echo $lang_name; ?>)</div>
...
link.id,
link.name_en,
link.name_$short_name as others,
link.publish as l_p,
link_type.name
FROM
link 
INNER JOIN link_type
ON link.link_type_id = link_type.id";
	$rsd = mysql_query($sql);
	...
	while($row = mysql_fetch_array($rsd)){
		...
		?>
		<div id="field_content">
			<div id="action" onclick="edit_button_click('<?php echo $row['id'];  ?>');">Edit</div>
			<div id="action">
			<?php
			$check = "";
			$value = "";
			if($row['l_p'] == 1) {
			 ...
			 ?>
			<input id="<?php echo $row['id'];  ?>" name="" type="checkbox" value="<?php echo $value; ?>" <?php echo $check; ?> onclick="item_check_uncheck('<?php echo $row['id'];  ?>');" />
			
			</div>
			<div id="field_menu_type">
		   <?php echo $row['name'];  ?>
		</div>
			<div id="tittle"><?php echo $row['id'];  ?></div>
...
function edit_button_click(id){   
		...
		$.ajax({
				type: 'POST',
				url: 'settings/link/link_edit.php',	
				data: "id="+ selected_id
				+"& action="+ "edit" ,
				success: function(data) {
						//alert(data);
