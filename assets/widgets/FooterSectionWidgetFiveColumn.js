 ...
 */
class FooterSectionWidgetFiveColumn extends WidgetBase {
  getName() {
    return 'footer-section-five-column';
  }
  ...
  registerControls() {
    ...
    // ===== COLUMN BRANDS =====
    this.startControlsSection('column_brands_section', {
      label: 'Column - Brands',
      tab: 'content'
    });

    this.addControl('col_brands_title', {
      type: 'text',
      label: 'Title',
      default_value: 'Our Brands',
      placeholder: 'Enter title'
    });

    this.addControl('brands_list', {
      type: 'repeater',
      label: 'Brands',
      default_value: [],
      fields: [
  ...
  render() {
    ...
    const colBrandsTitle = this.getSetting('col_brands_title', 'Our Brands');
    const brandsList = this.getSetting('brands_list', []);
    ...
    if (brandsList && brandsList.length > 0) {
      brandsHTML = brandsList.map(item => {
        if (!item.image || !item.image.url) return '';
        return `<a href="${item.link || '#'}"><img src="${item.image.url}" alt="Brand" loading="lazy"></a>`;
      }).join('');
    }
    ...
    const outputHtml = `
      ...
      <div class="footer-top-widgets">
        ...
        <!-- Column Brands -->
        <div class="footer-widget">
          <h3 style="color: ${headingColor};">${this.escapeHtml(colBrandsTitle)}</h3>
          ${brandsHTML ? `<div class="footer-gallery footer-brands">${brandsHTML}</div>` : ''}
        </div>
...
window.elementorWidgetManager.registerWidget(FooterSectionWidgetFiveColumn);
