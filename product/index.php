<?php
// product/index.php - Dedicated Product Page handler
...
$scriptName = $_SERVER['SCRIPT_NAME']; // /new-cms/product/index.php
// We need the web root path for CMS_ROOT.
// If script is /new-cms/product/index.php, CMS_ROOT should be /new-cms
...
// Get Slug
// .htaccess should pass ?slug=...
// or we parse request URI
$requestUri = $_SERVER['REQUEST_URI'];
// Remove Query String
$uriPath = strtok($requestUri, '?');
// Expecting /new-cms/product/slug
$slug = '';
// Simple parsing: get segment after 'product/'
$parts = explode('/product/', $uriPath);
if (count($parts) > 1) {
    $slug = trim($parts[1], '/');
}
// Fallback to GET
...
if ($slug) {
    $stmt = $pdo->prepare("SELECT name, description FROM products WHERE slug = ?");
    $stmt->execute([$slug]);
    $prod = $stmt->fetch();
    if ($prod) {
        $metaTitle = $prod['name'];
    ...
    <body>
        ...
        <script>
            ...
            window.SITE_DATA = {
                ...
                page: <?php echo $pageContent; ?>, // The Single Product Widget
